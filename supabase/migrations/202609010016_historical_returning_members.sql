-- Allow season rollover to reuse a stable manager identity from any earlier
-- league season. The latest active roster remains a UI default, but a
-- commissioner may bring back someone who skipped one or more seasons.
...
begin
  ...
  if member_count < 2 or mod(member_count, 2) <> 0 then
    raise exception using
      errcode = '22023',
      message = 'A season must include an even number of teams between 2 and 64.';
  end if;
  if playoff_spots_value > member_count then
    raise exception using
      errcode = '22023',
      message = 'Playoff spots cannot exceed the number of teams.';
  end if;
  ...
  join public.league_members source_member
   ...
   and source_member.season < p_target_season
  where configured.value->>'source_member_id' is not null;

  if requested_returning_count <> resolved_returning_count then
    raise exception using
      errcode = 'P0002',
      message = 'A selected returning player is not part of this league history.';
  end if;
  ...
  if exists (
    ...
    raise exception using
      errcode = '22023',
      message = 'A returning manager was selected more than once.';
  end if;
